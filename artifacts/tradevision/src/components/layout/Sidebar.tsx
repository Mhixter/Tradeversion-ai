...
export function Sidebar({ onClose }: { onClose?: () => void }) {
  ...
  const { role, canViewBilling, canManageMembers, canManageBots, canCreateStrategies, canManageRisk, inCompany, isAdmin, isOwner } = useCompanyRole();
  ...
  return (
    <div className="w-[160px] shrink-0 bg-sidebar border-r border-border flex flex-col h-full overflow-y-auto overflow-x-hidden">
      ...
      <div className="px-2 pb-3 border-t border-border flex flex-col gap-2 pt-2 shrink-0">
        {/* Admin Portal link — only for owner / admin */}
        {(isOwner || isAdmin) && (
          <a
        ...
        <Link href="/billing" onClick={onClose}>
          <div className="bg-gradient-to-br from-lime-600 to-emerald-700 rounded-xl p-3 text-white cursor-pointer hover:opacity-90 transition-opacity">
